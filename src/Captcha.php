...
class Captcha
{
    protected $im = null; // 验证码图片实例
    protected $color = null; // 验证码字体颜色
     ...
     */
    protected function configure(string $type = null): void
    {
        if (is_null($type)) {
            $config = $this->config->get('captcha', []);
        } else {
            $config = $this->config->get('captcha.' . $type, []);
        }
    ...
    /**
     * 输出验证码
     * 默认并把验证码的值保存的session中，api 存 cache
     * @param null|string $type
     * @param bool $api
     */
    public function create(string $type = 'default', bool $api = false)
    {
        $this->configure($type);

        $generator = $this->generate($type, $api);
        ...
        if ($api == true) {
            return [
                'phrase' => config('app.debug') ? $generator['phrase'] : '', // 最终结果值
                'hash' => $generator['key'],
                'captcha' => $this->base64Img($content)
            ];
        }
     ...
     */
    public function base64Img($content)
    {
     ...
     * @throws Exception
     */
    protected function generate(string $type = 'default', $api = false): array
    {
        $bag = '';
     ...
     * @return string
     */
    protected function get_cache_key(string $cache_name = '', $hash = '')
    {
        return $cache_name . md5($hash);
    }
     ...
     */
    public function check($type = 'default', string $code = '')
    {
        $cache_name = 'laravel_captcha' . $type ?? '';
        if (!$this->session->has($cache_name)) {
            return false;
        }
        ...
        if ($res) {
            $this->session->remove($cache_name);
        }
     ...
     * @return bool
     */
    public function check_api($type = 'default', $code = '', $hash = '')
    {
        $cache_name = 'laravel_captcha' . $type ?? '';
        $cache_key = $this->get_cache_key($cache_name, $hash);
     ...
     * @return string
     */
    public function src(string $type = 'default'): string
    {
        return url('captcha/' . $type ?? '');
    }
     ...
     * @return string
     */
    public function img(string $type = 'default', array $attrs = []): string
    {
        $attrs_str = '';
        ...
        if (empty($attrs_str)) {
            $attrs_str = 'id="captcha" alt="captcha"';
        }
        $attrs_str = trim($attrs_str);

        $src = $this->src($type);

        return new HtmlString("<img src='{$src}' " . $attrs_str . " onclick='this.src=\"{$src}?\"+Math.random();' />");
    }
}
